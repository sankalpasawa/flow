...
9. **Configure shared infrastructure** — read `asawa-inc/holding/AI-PROVIDERS.md` and `EXTERNAL-SYSTEMS.md`:
   - Select AI provider from approved list based on use case + cost constraints
   - Copy `asawa-inc/holding/templates/ai-provider.ts` to company's `src/lib/ai.ts`
   ...
   - Follow override rules from `asawa-inc/holding/OVERRIDE-RULES.md`
